...
_GRAPH_QUALITY_RULES = (
    ...
    "・通貨単位: 日本円（JPY）\n"
    "\n【来店・人数の数え方（必須・厳守）】\n"
    "・df は明細(商品1行=1レコード)粒度。来店や人数を数える前に必ず来店単位へ集約すること。\n"
    "・来店組数 = df['来店ID'].nunique()。★伝票番号でカウントしてはいけない（使い回しで激しく過少になる）。\n"
    "・来店人数 = df.drop_duplicates('来店ID')['人数'].sum()。★明細行のまま df['人数'].sum() をしてはいけない（重複で激しく過大になる）。\n"
    "・客単価・滞在時間など1来店あたりの指標も、必ず来店IDで重複除去してから集計する。\n"
    "・日別/店舗別に数える場合も、まず来店IDで一意化し、その代表行で日付・店舗を判定すること。\n"
)
