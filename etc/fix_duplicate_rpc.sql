-- ============================================================
-- fix_duplicate_rpc.sql
-- orders / order_items テーブルの重複データ問題を RPC レベルで解消する
-- Supabase SQL Editor で実行してください
-- ============================================================

-- ────────────────────────────────────────────
-- Step 1: 重複の確認クエリ（実行して問題を把握してください）
-- ────────────────────────────────────────────
SELECT
    '① visits'      AS table_name,  COUNT(*) AS row_count FROM visits
UNION ALL
SELECT '② orders',                  COUNT(*) FROM orders
UNION ALL
SELECT '③ order_items',             COUNT(*) FROM order_items
UNION ALL
SELECT '④ visits あたり平均 orders',
    ROUND(COUNT(*)::NUMERIC / (SELECT COUNT(*) FROM visits), 1)
FROM orders
UNION ALL
SELECT '⑤ visits あたり平均 order_items',
    ROUND(COUNT(*)::NUMERIC / (SELECT COUNT(*) FROM visits), 1)
FROM order_items;

-- ────────────────────────────────────────────
-- Step 2: 重複の多い伝票 TOP10
-- ────────────────────────────────────────────
SELECT
    v.receipt_no,
    s.store_name,
    COUNT(o.order_id)    AS order_count,
    COUNT(oi.order_item_id) AS item_count
FROM visits v
INNER JOIN stores s ON s.store_id = v.store_id
LEFT JOIN  orders o ON o.visit_id = v.visit_id
LEFT JOIN  order_items oi ON oi.order_id = o.order_id
GROUP BY v.receipt_no, s.store_name
ORDER BY order_count DESC
LIMIT 10;

-- ────────────────────────────────────────────
-- Step 3: get_izakaya_sales RPC を「重複除去版」に更新
--   DISTINCT ON (visit_id, item_name_raw, quantity, unit_price) で
--   同一来店・同一商品の重複行を DB 側で排除する
--   ※ 正常データ（同一来店で同一商品を異なる注文セッションで追加した場合）
--     も 1 行に集約されるが、数量(quantity)が同じなら実質的な重複とみなす
-- ────────────────────────────────────────────
DROP FUNCTION IF EXISTS get_izakaya_sales(TEXT, TEXT, INTEGER[]);

CREATE OR REPLACE FUNCTION get_izakaya_sales(
    p_start_date TEXT,
    p_end_date   TEXT,
    p_store_ids  INTEGER[] DEFAULT NULL
)
RETURNS TABLE(
    receipt_no           TEXT,
    order_time           TIMESTAMPTZ,
    visit_time           TIMESTAMPTZ,
    leave_time           TIMESTAMPTZ,
    party_size           INTEGER,
    customer_layer       TEXT,
    store_name           TEXT,
    shop_code            TEXT,
    item_name_raw        TEXT,
    quantity             INTEGER,
    unit_price           NUMERIC,
    temperature_2m_max   NUMERIC,
    temperature_2m_min   NUMERIC,
    temperature_2m_mean  NUMERIC,
    precipitation_sum    NUMERIC,
    weathercode          SMALLINT,
    weather_label        TEXT
)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
    SET LOCAL statement_timeout = '0';

    RETURN QUERY
    -- CTE で重複除去: 同一来店・同一商品（name+qty+price）の最初の 1 行のみ保持
    WITH deduped AS (
        SELECT DISTINCT ON (v.visit_id, oi.item_name_raw, oi.quantity, oi.unit_price)
            v.receipt_no,
            o.order_time,
            v.visit_time,
            v.leave_time,
            v.party_size::INTEGER            AS party_size,
            v.customer_layer,
            s.store_name,
            s.shop_code,
            oi.item_name_raw,
            oi.quantity::INTEGER             AS quantity,
            oi.unit_price,
            dw.temperature_2m_max,
            dw.temperature_2m_min,
            dw.temperature_2m_mean,
            dw.precipitation_sum,
            dw.weathercode,
            dw.weather_label
        FROM visits v
        INNER JOIN stores       s  ON s.store_id    = v.store_id
        INNER JOIN orders       o  ON o.visit_id    = v.visit_id
        INNER JOIN order_items  oi ON oi.order_id   = o.order_id
        LEFT  JOIN daily_weather dw ON dw.location_id = s.location_id
                                    AND dw.date = (v.visit_time AT TIME ZONE 'Asia/Tokyo')::DATE
        WHERE v.visit_time >= p_start_date::TIMESTAMPTZ
          AND v.visit_time <  (p_end_date::DATE + INTERVAL '1 day')::TIMESTAMPTZ
          AND oi.line_type = 'M'
          AND (p_store_ids IS NULL OR v.store_id = ANY(p_store_ids))
        -- DISTINCT ON は ORDER BY の先頭と一致させる必要がある
        ORDER BY v.visit_id, oi.item_name_raw, oi.quantity, oi.unit_price, o.order_time
    )
    SELECT
        d.receipt_no,
        d.order_time,
        d.visit_time,
        d.leave_time,
        d.party_size,
        d.customer_layer,
        d.store_name,
        d.shop_code,
        d.item_name_raw,
        d.quantity,
        d.unit_price,
        d.temperature_2m_max,
        d.temperature_2m_min,
        d.temperature_2m_mean,
        d.precipitation_sum,
        d.weathercode,
        d.weather_label
    FROM deduped d
    ORDER BY d.visit_time, d.receipt_no, d.order_time;
END;
$$;

GRANT EXECUTE ON FUNCTION get_izakaya_sales(TEXT, TEXT, INTEGER[]) TO anon;
GRANT EXECUTE ON FUNCTION get_izakaya_sales(TEXT, TEXT, INTEGER[]) TO authenticated;

-- ────────────────────────────────────────────
-- Step 4: 修正後の確認
-- ────────────────────────────────────────────
-- 以下を実行して、修正前後の行数の変化を確認してください
-- SELECT COUNT(*) FROM get_izakaya_sales('2024-09-01', '2024-09-30');
-- 期待値: ~283,000 / 4.6 ≈ 61,000 行程度（全店舗9月分）
