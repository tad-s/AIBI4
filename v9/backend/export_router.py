...
def _build_excel(analyses: list[dict], chat_analyses: list[dict], evidence_log: list[dict], df, summary_text: str) -> io.BytesIO:
    import openpyxl
    ...
    # ─── Evidence log sheet ───
    if evidence_log:
        ws_e = wb.create_sheet("エビデンス")
        ws_e.column_dimensions["A"].width = 22
        ws_e.column_dimensions["B"].width = 28
        ws_e.column_dimensions["C"].width = 60
        ws_e.column_dimensions["D"].width = 18
        set_hdr(ws_e, 1, 1, "時刻")
        set_hdr(ws_e, 1, 2, "種別")
        set_hdr(ws_e, 1, 3, "対象/内容")
        set_hdr(ws_e, 1, 4, "ステータス")
        row = 2
        for ev in evidence_log:
            ws_e.cell(row=row, column=1, value=str(ev.get("ts", "")))
            ws_e.cell(row=row, column=2, value=str(ev.get("type", "")))
            target = ev.get("title") or ev.get("question") or ev.get("effective_message") or ev.get("evidence_note") or ""
            ws_e.cell(row=row, column=3, value=str(target)[:1000])
            ws_e.cell(row=row, column=4, value=str(ev.get("status", "")))
            for c in range(1, 5):
                ws_e.cell(row=row, column=c).alignment = Alignment(wrap_text=True, vertical="top")
            row += 1
...
def export_excel(sid: str):
    ...
    try:
        buf = _build_excel(analyses, chat_analyses, s.get("evidence_log") or [], df, summary_text)
