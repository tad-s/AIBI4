...
# ── V9 追加分析: 注文導線分析ユーティリティ ──
def _order_flow_df(order_df: pd.DataFrame | None) -> pd.DataFrame:
    if order_df is None or order_df.empty:
        return pd.DataFrame()
    needed = {"注文ID", "商品リスト", "客単価"}
    if not needed.issubset(set(order_df.columns)):
        return pd.DataFrame()
    rows = []
    for _, row in order_df.iterrows():
        items = [str(x) for x in (row.get("商品リスト") or []) if str(x) and str(x) != "nan"]
        if not items:
            continue
        order_time = row.get("来店時間") if "来店時間" in order_df.columns else None
        for step, item in enumerate(items, start=1):
            rows.append({
                ...
                "客単価": row.get("客単価"),
                "商品数": len(items),
                "時間帯": row.get("時間帯") if "時間帯" in order_df.columns else None,
                "人数": row.get("人数") if "人数" in order_df.columns else None,
                "来店時間": order_time,
            })
    return pd.DataFrame(rows)


def _pair_counter_by_sequence(order_df: pd.DataFrame | None) -> Counter:
    cnt = Counter()
    if order_df is None or "商品リスト" not in order_df.columns:
        return cnt
    for items in order_df["商品リスト"]:
        items = [str(x) for x in (items or []) if str(x) and str(x) != "nan"]
...
def _as_table(records: list[dict]) -> list[dict]:
    return records if records else []


# ── 分析⑦ 注文の流れ ──
def analysis_7_order_flow(order_df: pd.DataFrame | None) -> list[dict]:
    flow = _order_flow_df(order_df)
    if flow.empty or flow["注文ID"].nunique() < 10:
        return [{
            "title": "分析⑦ 注文の流れ",
            "image_b64": _placeholder_b64("注文順序を分析できるデータが不足しています"),
            "insight": "同一伝票内の商品リストが不足しているため、注文ステップ別分析を実行できません。",
            "insights": ["注文順序または商品明細が不足しています"],
            "advice": ["注文日時・商品明細が伝票単位で取得できる状態にしてください"],
            "table": None,
        }]
    step_counts = (
        flow.groupby(["step", "商品名"]).size().reset_index(name="件数")
        .sort_values(["step", "件数"], ascending=[True, False])
    )
    top_steps = step_counts.groupby("step").head(5)
    first_items = top_steps[top_steps["step"] == 1].head(8)
    seq = _pair_counter_by_sequence(order_df).most_common(10)
    seq_df = pd.DataFrame([{"遷移": f"{a} → {b}", "件数": c} for (a, b), c in seq])

    fig, axes = plt.subplots(1, 2, figsize=(12, 4.2))
    axes[0].barh(first_items["商品名"][::-1], first_items["件数"][::-1], color="#5b9bd5")
    axes[0].set_title("初期注文（1品目）Top")
    axes[0].set_xlabel("件数")
    if not seq_df.empty:
        axes[1].barh(seq_df["遷移"][::-1], seq_df["件数"][::-1], color="#e67e22")
    axes[1].set_title("注文遷移 Top10")
    axes[1].set_xlabel("件数")
    plt.tight_layout()

    total_orders = order_df.shape[0]
    one_item_rate = (order_df["商品数"].eq(1).mean() * 100) if "商品数" in order_df.columns else 0
    top_first = first_items.iloc[0]["商品名"] if not first_items.empty else "-"
    return [{
        "title": "分析⑦ 注文の流れ",
        "image_b64": _fig_to_b64(fig),
        "insight": f"初期注文の最多は **{top_first}**。1品のみで止まる伝票は {one_item_rate:.1f}% です。",
        "insights": [
            f"初期注文で最も多い商品は **{top_first}**",
            f"1品のみで止まる伝票比率は **{one_item_rate:.1f}%**",
            "注文遷移Topは追加提案の候補になります",
        ],
        "advice": [
            "初期注文直後に2品目候補を提示し、1品止まりを減らす",
            "遷移Topの商品をレコメンド・卓上POP・スタッフ声かけに使う",
            "初期注文Top商品の近くに追加注文候補を配置する",
        ],
        "table": _as_table(top_steps.to_dict("records") + seq_df.to_dict("records")),
    }]


# ── 分析⑧ 売れるメニューの組み合わせ ──
def analysis_8_menu_combinations(order_df: pd.DataFrame | None) -> list[dict]:
    if order_df is None or "商品リスト" not in order_df.columns or len(order_df) < 10:
        return [{"title": "分析⑧ 売れるメニューの組み合わせ", "image_b64": _placeholder_b64("商品組み合わせ分析に必要なデータが不足"), "insight": "商品リストが不足しています。", "insights": ["商品リスト不足"], "advice": ["商品明細を取得してください"], "table": None}]
    same = Counter()
    for items in order_df["商品リスト"]:
        items = list(dict.fromkeys([str(x) for x in (items or []) if str(x) and str(x) != "nan"]))
        for a, b in combinations(items, 2):
            same[tuple(sorted((a, b)))] += 1
    seq = _pair_counter_by_sequence(order_df)
    same_df = pd.DataFrame([{"組み合わせ": f"{a} × {b}", "件数": c} for (a, b), c in same.most_common(10)])
    seq_df = pd.DataFrame([{"遷移": f"{a} → {b}", "件数": c} for (a, b), c in seq.most_common(10)])
    fig, axes = plt.subplots(1, 2, figsize=(12, 4.2))
    if not same_df.empty:
        axes[0].barh(same_df["組み合わせ"][::-1], same_df["件数"][::-1], color="#3b6ee8")
    axes[0].set_title("同時注文ペア Top10")
    axes[0].set_xlabel("件数")
    if not seq_df.empty:
        axes[1].barh(seq_df["遷移"][::-1], seq_df["件数"][::-1], color="#e74c3c")
    axes[1].set_title("連続注文ペア Top10")
    axes[1].set_xlabel("件数")
    plt.tight_layout()
    top_pair = same_df.iloc[0]["組み合わせ"] if not same_df.empty else "-"
    return [{
        "title": "分析⑧ 売れるメニューの組み合わせ",
        "image_b64": _fig_to_b64(fig),
        "insight": f"最も強い同時注文ペアは **{top_pair}** です。",
        "insights": [f"同時注文で最も多いペアは **{top_pair}**", "連続注文ペアは追加提案の順序設計に使えます", "同時注文と連続注文を分けて見ることでセット化と追加提案を分離できます"],
        "advice": ["同時注文Topをセットメニュー候補にする", "連続注文Topを次のおすすめとして表示する", "ドリンク×フードの組み合わせをPOP・レコメンド枠に反映する"],
        "table": _as_table(same_df.to_dict("records") + seq_df.to_dict("records")),
    }]


# ── 分析⑨ 初期注文の影響 ──
def analysis_9_initial_order_impact(order_df: pd.DataFrame | None) -> list[dict]:
    if order_df is None or "商品リスト" not in order_df.columns or len(order_df) < 10:
        return [{"title": "分析⑨ 初期注文の影響", "image_b64": _placeholder_b64("初期注文分析に必要なデータが不足"), "insight": "商品リストが不足しています。", "insights": ["初期注文を特定できません"], "advice": ["伝票単位の商品明細を取得してください"], "table": None}]
    odf = order_df.copy()
    odf["初期注文"] = odf["商品リスト"].apply(lambda xs: str(xs[0]) if isinstance(xs, list) and xs else "不明")
    agg = odf.groupby("初期注文").agg(件数=("客単価", "count"), 平均客単価=("客単価", "mean"), 平均商品数=("商品数", "mean")).reset_index()
    agg = agg[agg["件数"] >= 3].sort_values("平均商品数", ascending=False).head(12)
    fig, ax1 = plt.subplots(figsize=(10, 4.2))
    ax1.bar(agg["初期注文"], agg["平均商品数"], color="#5b9bd5", label="平均商品数")
    ax1.set_ylabel("平均商品数")
    ax1.set_title("初期注文別 平均商品数")
    ...
    return [{
        "title": "分析⑨ 初期注文の影響",
        "image_b64": _fig_to_b64(fig),
        "insight": f"注文数を最も伸ばす初期注文は **{top['初期注文'] if top is not None else '-'}** です。",
        "insights": [f"平均商品数が最大の初期注文は **{top['初期注文'] if top is not None else '-'}**", "初期注文はその後の注文点数に影響します", "件数が少ない初期商品は過大評価に注意が必要です"],
        "advice": ["注文数を伸ばす初期商品をファーストオーダー候補にする", "初期1品だけの客には早めに相性商品を提示する", "初期注文別にセット導線を設計する"],
        "table": agg.to_dict("records"),
    }]


# ── 分析⑩ 注文の連鎖条件 ──
def analysis_10_chain_conditions(order_df: pd.DataFrame | None) -> list[dict]:
    if order_df is None or "商品数" not in order_df.columns or len(order_df) < 10:
        return [{"title": "分析⑩ 注文の連鎖条件", "image_b64": _placeholder_b64("注文連鎖分析に必要なデータが不足"), "insight": "商品数が不足しています。", "insights": ["商品数不足"], "advice": ["商品明細を取得してください"], "table": None}]
    odf = order_df.copy()
    bins = [0, 1, 2, 3, 5, 999]
    labels = ["1品", "2品", "3品", "4-5品", "6品以上"]
    odf["品数帯"] = pd.cut(odf["商品数"], bins=bins, labels=labels)
    grp = odf.groupby("品数帯", observed=True).agg(件数=("客単価", "count"), 平均客単価=("客単価", "mean")).reset_index()
    fig, ax = plt.subplots(figsize=(8, 4))
    ax.bar(grp["品数帯"].astype(str), grp["件数"], color="#6c42f0")
    ax.set_title("注文品数帯別 件数")
    ax.set_ylabel("伝票数")
    plt.tight_layout()
    two_plus = (odf["商品数"] >= 2).mean() * 100
    three_plus = (odf["商品数"] >= 3).mean() * 100
    return [{
        "title": "分析⑩ 注文の連鎖条件",
        "image_b64": _fig_to_b64(fig),
        "insight": f"2品以上に進む伝票は **{two_plus:.1f}%**、3品以上は **{three_plus:.1f}%** です。",
        "insights": [f"2品以上に進む比率: **{two_plus:.1f}%**", f"3品以上に進む比率: **{three_plus:.1f}%**", "品数帯ごとの件数分布から注文連鎖の山を確認できます"],
        "advice": ["1品止まりを減らす施策を優先する", "2品目注文後に3品目候補を提示する", "品数帯別に客単価差を確認し、追加提案の優先度を決める"],
        "table": grp.to_dict("records"),
    }]


# ── 分析⑪ 時間帯別の違い ──
def analysis_11_timeband_differences(order_df: pd.DataFrame | None) -> list[dict]:
    if order_df is None or "時間帯" not in order_df.columns or len(order_df) < 10:
        return [{"title": "分析⑪ 時間帯別の違い", "image_b64": _placeholder_b64("時間帯分析に必要なデータが不足"), "insight": "時間帯列が不足しています。", "insights": ["時間帯を算出できません"], "advice": ["来店時間または注文日時を取得してください"], "table": None}]
    odf = order_df.copy()
    ...
    return [{
        "title": "分析⑪ 時間帯別の違い",
        "image_b64": _fig_to_b64(fig),
        "insight": f"平均商品数が最も高い時間帯は **{int(top_qty['時間帯'])}時台** です。",
        "insights": [f"注文点数が伸びる時間帯は **{int(top_qty['時間帯'])}時台**", "時間帯により注文点数と客単価の山が異なります", "ピーク時間帯と軽い利用時間帯を分けて施策設計できます"],
        "advice": ["注文点数が伸びる時間帯に高単価商品の提案を強化する", "軽い利用時間帯には低負荷な追加商品を提示する", "時間帯別におすすめ商品の表示順を変える"],
        "table": grp.to_dict("records"),
    }]


# ── 分析⑫ 注文が止まるポイント ──
def analysis_12_stop_points(order_df: pd.DataFrame | None) -> list[dict]:
    if order_df is None or "商品数" not in order_df.columns or len(order_df) < 10:
        return [{"title": "分析⑫ 注文が止まるポイント", "image_b64": _placeholder_b64("停止ポイント分析に必要なデータが不足"), "insight": "商品数が不足しています。", "insights": ["停止ポイントを推定できません"], "advice": ["商品明細を取得してください"], "table": None}]
    odf = order_df.copy()
    stop = odf["商品数"].clip(upper=8).value_counts().sort_index().reset_index()
    stop.columns = ["最終品数", "件数"]
    if "商品リスト" in odf.columns:
        odf["最終商品"] = odf["商品リスト"].apply(lambda xs: str(xs[-1]) if isinstance(xs, list) and xs else "不明")
        last_df = odf["最終商品"].value_counts().head(10).reset_index()
        last_df.columns = ["最終商品", "件数"]
    else:
        last_df = pd.DataFrame()
    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    axes[0].bar(stop["最終品数"].astype(str), stop["件数"], color="#95a5a6")
    axes[0].set_title("注文が止まった品数")
    axes[0].set_ylabel("伝票数")
    if not last_df.empty:
        axes[1].barh(last_df["最終商品"][::-1], last_df["件数"][::-1], color="#27ae60")
    axes[1].set_title("最終注文商品 Top10")
    plt.tight_layout()
    top_stop = stop.loc[stop["件数"].idxmax(), "最終品数"]
    return [{
        "title": "分析⑫ 注文が止まるポイント",
        "image_b64": _fig_to_b64(fig),
        "insight": f"注文が最も止まりやすい品数は **{top_stop}品目** です。",
        "insights": [f"最も多い停止ポイントは **{top_stop}品目**", "最終注文商品は『締め』または『離脱前の商品』の候補です", "停止ポイント前の提案が注文数増加の余地になります"],
        "advice": [f"{top_stop}品目到達前に追加候補を提示する", "最終注文Topの商品後に相性の良い追加商品を提案する", "ラストオーダー前の声かけタイミングを停止ポイントに合わせる"],
        "table": _as_table(stop.to_dict("records") + last_df.to_dict("records")),
    }]
...
def run_all_analyses(df: pd.DataFrame) -> list[dict]:
    ...
    for fn in [
        ...
        analysis_7_order_flow,
        analysis_8_menu_combinations,
        analysis_9_initial_order_impact,
        analysis_10_chain_conditions,
        analysis_11_timeband_differences,
        analysis_12_stop_points,
    ]:
        try:
