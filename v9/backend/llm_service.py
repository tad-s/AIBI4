...
_GRAPH_QUALITY_RULES = (
    ...
    "\n【データ列の意味】\n"
    "・来店ID: 1来店(組)の正準一意キー（店舗・伝票番号・来店/退店時刻の複合）\n"
    "・伝票番号: レシート番号。店舗・日をまたいで使い回されるため単独では来店を識別できない\n"
    "・合計金額(税込): 伝票合計（客単価）\n"
    "・単価: 1商品の販売価格\n"
    "・数量: 商品数量\n"
    "・人数: 1来店グループの人数\n"
    ...
    "・商品名: 注文された商品名\n"
    "・通貨単位: 日本円（JPY）\n"
    "\n【来店・人数の数え方（最重要・必ずこの手順でコードを書く）】\n"
    "df は明細(商品1行=1レコード)粒度。来店/人数/客単価は必ず列『来店ID』で一意化してから数える。\n"
    "★伝票番号での単独カウント禁止（店舗・日で使い回され激しく過少）。★明細のまま df['人数'].sum() 禁止（重複で激しく過大）。\n"
    "来店・人数・日別・特定日の質問では、必ず次のコードを出発点にすること（列名はこのまま存在する）:\n"
    "```\n"
    "v = df.drop_duplicates('来店ID').copy()            # 1来店=1行に集約\n"
    "v['来店時間'] = pd.to_datetime(v['来店時間'], errors='coerce')\n"
    "v['日付'] = v['来店時間'].dt.date\n"
    "```\n"
    "・全体:      来店組数 = len(v)              来店人数 = int(v['人数'].sum())\n"
    "・特定日:    target = pd.Timestamp('2026-05-01').date(); day = v[v['日付']==target]; 組数=len(day); 人数=int(day['人数'].sum())\n"
    "・日別人数:  daily = v.groupby('日付')['人数'].sum()     # 日別組数なら v.groupby('日付').size()\n"
    "★どの来店数の質問でも必ずグラフを1枚描くこと（数値だけで終わらせない・単一日でも必須）。\n"
    "【日別/期間の質問】daily をそのまま棒グラフに:\n"
    "    if len(daily) > 0:\n"
    "        fig, ax = plt.subplots(figsize=(11,4))\n"
    "        ax.bar([str(d) for d in daily.index], daily.values, color='skyblue')\n"
    "        ax.set_title('日別来店人数'); ax.set_xlabel('日付'); ax.set_ylabel('来店人数（人）'); plt.xticks(rotation=90)\n"
    "【特定日の質問】その日を含む月の日別推移を描き、対象日だけ色を変えて強調する:\n"
    "    month = v[(v['日付'] >= pd.Timestamp('2026-05-01').date()) & (v['日付'] <= pd.Timestamp('2026-05-31').date())]\n"
    "    daily = month.groupby('日付')['人数'].sum()\n"
    "    if len(daily) > 0:\n"
    "        colors = ['crimson' if d == target else 'skyblue' for d in daily.index]\n"
    "        fig, ax = plt.subplots(figsize=(11,4))\n"
    "        ax.bar([str(d) for d in daily.index], daily.values, color=colors)\n"
    "        ax.set_title('日別来店人数（対象日を赤で強調）'); ax.set_xlabel('日付'); ax.set_ylabel('来店人数（人）'); plt.xticks(rotation=90)\n"
    "・客単価・滞在時間など1来店あたりの指標も、必ず v（来店IDで一意化した後）で集計する。\n"
)
