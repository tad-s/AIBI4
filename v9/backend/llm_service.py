...
_GRAPH_QUALITY_RULES = (
    ...
    "・特定日:    day = v[v['日付'] == pd.Timestamp('2026-05-01').date()]; 組数=len(day); 人数=int(day['人数'].sum())\n"
    "・日別人数:  daily = v.groupby('日付')['人数'].sum()     # 日別組数なら v.groupby('日付').size()\n"
    "・グラフは上記 daily（や集計結果）を使い、len(daily)>0 を確認してから必ず1枚描画する:\n"
    "    if len(daily) > 0:\n"
    "        fig, ax = plt.subplots(figsize=(11,4))\n"
    "        ax.bar([str(d) for d in daily.index], daily.values)\n"
    "        ax.set_title('日別来店人数'); ax.set_xlabel('日付'); ax.set_ylabel('来店人数（人）')\n"
